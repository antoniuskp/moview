...
export class MovieService {
  ...
  movies(): Observable<any> {
    return this.http.get("https://ubaya.xyz/hybrid/160422065/moview/movie.php");
  }

  parseMovies(data: any) {
    let newData = data.map((d: any) => {
      const { id, poster, judul, genre, tanggal_rilis, sutradara, sinopsis, actors, reviews } = d;
      console.log(d);
      return {
        id:id, 
        poster: poster,
  ...
  addReview(movie_id: number, review: IReview) {
    const headers = new HttpHeaders({ 'Content-Type': 'application/x-www-form-urlencoded' });
    const body = new URLSearchParams();
    body.set('id', movie_id.toString());
    body.set('usernameuser', review.username);
    body.set('review', review.review);
    body.set('rating', review.rating.toString());
    body.set('date', review.date.toISOString().split('T')[0]);
    const urlEncodedData = body.toString();

    return this.http.post("https://ubaya.xyz/hybrid/160422065/moview/insert_review.php", urlEncodedData, { headers });
  }
  ...
  addMovie(p_poster: string,
    p_judul: string, p_genre: string,
    p_tanggal: string, p_sutradara: string,
    p_sinopsis: string, p_actors: Array<{ name: string, role: string }>) {
    const headers = new HttpHeaders({ 'Content-Type': 'application/x-www-form-urlencoded' });
    const body = new URLSearchParams();
    body.set('poster', p_poster);
    body.set('judul', p_judul);
    body.set('genre', p_genre);
    body.set('tanggal_rilis', p_tanggal);
    body.set('sutradara', p_sutradara);
    body.set('sinopsis', p_sinopsis);
    body.set('actors', p_actors.length > 0 ? JSON.stringify(p_actors) : 'null');
    const urlEncodedData = body.toString();
    return this.http.post("https://ubaya.xyz/hybrid/160422065/moview/insert_movie.php", urlEncodedData, { headers });
  }
  ...
  //   }
  // }
  editMovie(id: number, p_poster:
    string, p_judul: string, p_genre:
      string, p_tanggal: string, p_sutradara: string,
    p_sinopsis: string, p_actors: Array<{ name: string, role: string }>) {
    const headers = new HttpHeaders({ 'Content-Type': 'application/x-www-form-urlencoded' });
    const body = new URLSearchParams();
    ...
    body.set('sutradara', p_sutradara);
    body.set('sinopsis', p_sinopsis);
    body.set('actors', JSON.stringify(p_actors));
    const urlEncodedData = body.toString();

    return this.http.post("https://ubaya.xyz/hybrid/160422065/moview/update_movie.php", urlEncodedData, { headers });
  }
  ...
  deleteMovie(id: number) {
    ...
    return this.http.post("https://ubaya.xyz/hybrid/160422065/moview/delete_movie.php", urlEncodedData, { headers });
  }

  detailMovie(id: number): Observable<any> {
    return this.http.get("https://ubaya.xyz/hybrid/160422065/moview/detail_movie.php?id=" + id);
  }
  ...
  //   return this.http.post("https://ubaya.xyz/hybrid/160422065/detail_movie.php", urlEncodedData, { headers });
  // }

  parseDetailMovie(movie: any) {
    const { id, poster, judul, genre, tanggal_rilis, sutradara, sinopsis, actors, reviews } = movie;
    return {
      id:id,
      poster: poster,
      judul: judul,
